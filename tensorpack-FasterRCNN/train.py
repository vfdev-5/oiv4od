...
def predict(pred_func, input_file):
    ...
    output_filename = "pred_viz_{}".format(os.path.basename(input_file))
    cv2.imwrite(output_filename, viz)
    # tpviz.interactive_imshow(viz)
    print("Output predictions viz file is written at {}".format(output_filename))


class EvalCallback(Callback):
    def __init__(self, in_names, out_names, interval=100):
    ...
    def _before_train(self):
        eval_times = self.trainer.max_epoch // self.interval
        self.epochs_to_eval = set([self.interval * k for k in range(1, eval_times + 1)])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--logdir', help='log directory', default='output')
    parser.add_argument('--visualize', action='store_true', help='visualize intermediate results')
    parser.add_argument('--evaluate', help="Run evaluation on OpenImages. "
    ...
    now = datetime.now()
    args.logdir = args.logdir + "/{}".format(now.strftime("%Y%m%d_%H%M"))
    ...
    if args.visualize or args.evaluate or args.predict:
        ...
        # Dump config into logdir:
        conf_dict = cfg.to_dict()
        with open(os.path.join(args.logdir, "config.json"), "w") as h:
        ...
        callbacks = [
            ...
            EvalCallback(*MODEL.get_inference_tensor_names(), interval=cfg.TRAIN.EVAL_INTERVAL_EPOCH),
            PeakMemoryTracker(),
            EstimatedTimeLeft(),
            SessionRunTimeout(60000).set_chief_only(True),   # 3 minute timeout
        ]
        if not is_horovod:
